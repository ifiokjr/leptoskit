...
/// `TestElement` is a wrapper around `web_sys::HtmlElement`, we supply some
/// helper functions while implementing Deref<HtmlElement> so you can use any
/// `web_sys::HtmlElement` method on `TestElement`.
#[derive(Clone, Debug, PartialEq, Deref, DerefMut, From, Into)]
pub struct TestElement(pub HtmlElement);

impl TestElement {
	/// Click outside of an element. This will try to turn the parent node of
	/// the element into an `HtmlElement` and click on it. If the parent
	...
	/// Gets the text the user would see. There might be multiple nodes, this
	/// will display all text
	...
	/// If the element is a Input, `TextArea` or Select Element this will set
	/// the display value. Otherwise it will panic.
